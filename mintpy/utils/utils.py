...
#####################################################################
def transect_yx(z, atr, start_yx, end_yx, interpolation='nearest',get_lat_lon_points=True, prof_width=None):
    ...
    Parameters: z : (np.array) 2D data matrix
                ...
                get_lat_lon : Get the lat lon points along the profile
                prof_wdith : profile width in pixels 
    ...
    if 'Y_FIRST' in atr.keys():
        ...
        if get_lat_lon_points==True:
            print('Adding lat/lon points to profile')
    ...
    dist_line = np.hypot((xs - x0) * x_step,
                         (ys - y0) * y_step)
    # Added by Ollie Dec 21
    # See https://github.com/olliestephenson/MintPy/blob/ollie_mintpy_old/mintpy/utils/utils.py
...
def transect_lalo(z, atr, start_lalo, end_lalo, interpolation='nearest',get_lat_lon_points=True,prof_width=None):
    ...
    transect = transect_yx(z, atr, [y0, x0], [y1, x1], interpolation,get_lat_lon_points=get_lat_lon_points,prof_width=prof_width)
    return transect
