...
def estimate_S1AB_bias(mintpy_dir, ts_dis):
    ...
    s1a_date_list_file, s1b_date_list_file = get_s1ab_date_list_file(mintpy_dir, print_msg=False)
...
def get_s1ab_date_list_file(mintpy_dir, print_msg=True):
    ...
    if not os.path.isfile(s1a_date_list_file):
        ...
        if not os.path.isfile(safe_list_file):
            raise FileNotFoundError(f'No file found in: {safe_list_file}!')
