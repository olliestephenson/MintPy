...
####################################################################################
EXAMPLE = """example:
  s1ab_range_bias.py timeseriesRg_SET_ERA5.h5
  s1ab_range_bias.py timeseriesRg_SET_ERA5.h5 -m maskResInv.h5 -g inputs/geometryRadar.h5
"""
...
def create_parser():
    ...
    parser.add_argument('-b', '--bias-method', dest='bias_method', choices={'hardwired', 'data-driven'}, default='hardwired',
                        help='Bias estimation method (default: %(default)s):\n'
                             'hardwired   - use hardwired values from section VII-A in Yunjun et al. (2022)\n'
                             'data-driven - estimate from the input TS file, using the same method as in Yunjun et al. (2022)')
    parser.add_argument('--force', dest='force', action='store_true', help='Force to re-generate the S1Bias.h5 file.')
    parser.add_argument('--plot', dest='plot', action='store_true', help='Plot the data-driven estimation result.')
...
####################################################################################
def estimate_S1AB_range_bias(ts_file, mask_file=None, plot=False):
    ...
    bias_est[mask] = s1_utils.estimate_S1AB_bias(mintpy_dir, ts_data[:, mask])[0]
    ...
    if plot:
        plot_S1AB_range_bias_est(bias_list, bias_est, mask_list)

    return bias_list, bias_est, mask_list


def plot_S1AB_range_bias_est(bias_list, bias_est, mask_list):
    ...
    # output
    out_fig = os.path.join(work_dir, 's1ab_range_bias_map.pdf')
    ...
    # output
    out_fig = os.path.join(work_dir, 's1ab_range_bias_hist.pdf')
    #print('save figure to file', out_fig)
    #plt.savefig(out_fig, bbox_inches='tight', transparent=True, dpi=300)
...
def write_to_file(bias_list, geom_file, force=False):
...
def correct_S1AB_range_bias(ts_file, bias_file, ts_cor_file=None):
    """Correct for the S1A/B bias."""
    date_list = timeseries(ts_file).get_date_list()
    num_date = len(date_list)

    # date info for Sentinel-1B
    s1b_date_list_file = s1_utils.get_s1ab_date_list_file(os.path.dirname(os.path.dirname(bias_file)))[1]
...
def main(iargs=None):
    ...
    correct_S1AB_range_bias(inps.ts_file, bias_file, ts_cor_file=inps.ts_cor_file)
